...
int index_page(void *p, onion_request *req, onion_response *res) {
  ...
  if (strstr(sound_name, "/..") != NULL || strstr(sound_name, "../") != NULL) {
    snprintf(err_msg, PATH_MAX,
             "sound_name [%s] may try to escape from sound_repository_path",
             sound_name);
    ONION_WARNING(err_msg);
    return onion_shortcut_response(err_msg, HTTP_BAD_REQUEST, req, res);
  }
  ...
  if (enqueue(sound_realpath)) {
    ...
    if (qs == 0) { // i.e., before enqueue() the queue is empty, so we start a
                   // new handle_sound_name_queue thread.
      pthread_t my_thread;
      if (pthread_create(&my_thread, NULL, handle_sound_name_queue, NULL) !=
          0) {
        ONION_ERROR(
            "Failed to pthread_create() handle_sound_name_queue, reason: %s",
            strerror(errno));
      } else {
        if (pthread_detach(my_thread) == 0) {
          // We need to detach the thread so when handle_sound_name_queue()
          // returns, the thread resources will be fully released; otherwise, it
          // is suspected that some thread resources will NOT be released and we
          // will be unable to pthread_create() new thread a while later,
          // resulting in "Cannot allocate memory" error.
          ONION_INFO("handle_sound_name_queue pthread_create()'ed");
        } else {
          ONION_ERROR("handle_sound_name_queue pthread_create()'ed, but failed "
                      "to pthread_detach() it, reason: %s",
                      my_thread, strerror(errno));
        }
      }
    }
    snprintf(info_msg, PATH_MAX,
    ...
    pthread_mutex_unlock(&lock);
    snprintf(err_msg, PATH_MAX,
             "queue is full/server has not enough free memory, new sound "
             "discarded.");
    ONION_WARNING(err_msg);
    return onion_shortcut_response(err_msg, HTTP_BAD_REQUEST, req, res);
  }
...
int main(int argc, char **argv) {
  ...
  if (sound_repository_path == NULL ||
    ...
    ONION_ERROR("sound_repository [%s] is either NULL or too long",
                sound_repository_path);
    return 2;
  }
  if (log_path == NULL || pac_username == NULL || pac_passwd == NULL ||
      ssl_crt_path == NULL || ssl_key_path == NULL) {
    ONION_ERROR("Either root_app_log_path, username, passwd, ssl_crt_path, "
                "ssl_key_path is not set.");
    return 3;
  }
  ...
  if (dir) { // exist
    ...
    ONION_ERROR("sound_repository [%s] is inaccessible.",
                sound_repository_path);
    return 4;
  }
  ...
  if (argc == 2 && strcmp(argv[1], "--debug") == 0) {
    fprintf(stderr, "Debug mode enabled, the only change is that log will be "
                    "sent to stderr instead of a log file\n");
  } else {
  ...
  // We set this to a large number, hoping the client closes the connection
  // itself If the server times out before client does, GnuTLS complains "The
  ...
  ONION_INFO("Public address client listening on %s:%s",
             json_object_get_string(root_app_interface),
             json_object_get_string(root_app_port));
  onion_listen(o);
  onion_free(o);
  finalize_queue();
  ...
  freopen(
      "/dev/tty", "a",
      stderr); // may have side effects, but fit our purpose for the time being.
  return 0;
}
